...
class ObjectAddressSimulation:
    ...
    def store_addr_to_conn_handle(self, conn_handle: int, addr: int) -> None:
        if conn_handle not in self.connections:
            self.connections[conn_handle] = set()
        self.connections[conn_handle].add(addr)

    def get_addr_of_conn_handle(self, conn_handle: int) -> set:
        return self.connections[conn_handle] if conn_handle in self.connections else set()

    def clear_addr_of_conn_handle(self, conn_handle: int) -> None:
        self.connections[conn_handle] = set()
